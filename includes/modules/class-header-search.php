 ...
 * Header Search Class
 */
class Wellington_Pro_Header_Search {
	...
	static function setup() {
		...
		add_action( 'wellington_header_search', array( __CLASS__, 'add_header_search' ) );
		...
		add_filter( 'wellington_hide_elements', array( __CLASS__, 'hide_header_search' ) );
	}
	...
	static function enqueue_script() {
		...
		if ( ( true === $theme_options['header_search'] || is_customize_preview() ) && ! self::is_amp() ) :
			...
			wp_enqueue_script( 'wellington-pro-header-search', WELLINGTON_PRO_PLUGIN_URL . 'assets/js/header-search.min.js', array(), '20220121', true );
	...
	static function add_header_search() {
		...
		if ( true === $theme_options['header_search'] || is_customize_preview() ) : ?>
			...
			<div class="header-search">
				...
				<button class="header-search-icon" aria-label="<?php esc_attr_e( 'Open search form', 'wellington-pro' ); ?>" aria-expanded="false" aria-controls="header-search-dropdown" <?php self::amp_search_toggle(); ?>>
	...
	static function header_search_settings( $wp_customize ) {
		...
		$wp_customize->add_control( new Wellington_Customize_Header_Control(
			$wp_customize, 'wellington_theme_options[header_search_title]', array(
				'label'    => esc_html__( 'Header Search', 'wellington-pro' ),
		...
		$wp_customize->add_setting( 'wellington_theme_options[header_search]', array(
			'default'           => false,
			'type'              => 'option',
...
// Run Class.
add_action( 'init', array( 'Wellington_Pro_Header_Search', 'setup' ) );
